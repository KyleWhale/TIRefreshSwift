...
public class DefaultGTMLoadMoreFooter: GTMLoadMoreFooter, SubGTMLoadMoreFooterProtocol {
    ...
    var idleImage: UIImage? {
        didSet {
            if let idleImg = idleImage {
                self.pullingIndicator.image = idleImg.withRenderingMode(.alwaysTemplate)
            }
        }
    }
    ...
    lazy var pullingIndicator: UIImageView = {
        ...
        if let img = self.idleImage {
            ...
            pindicator.image = UIImage(named: "arrow_down", in: Bundle(for: GTMLoadMoreFooter.self), compatibleWith: nil)?.withRenderingMode(.alwaysTemplate)
        }
        pindicator.tintColor = .white
        return pindicator
    }()
